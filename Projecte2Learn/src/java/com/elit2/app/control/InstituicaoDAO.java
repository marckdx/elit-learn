...
public class InstituicaoDAO {
    ...
    public ArrayList<Instituicao> getInstituicoes() throws Exception {
        ...
        while (rs.next()) {
            Instituicao inst = new Instituicao(rs.getInt("cd_instituicao"), rs.getString("nm_instituicao"));
            insts.add(inst);
        }
        con.close();
        stmt.close();
    ...
    public ArrayList<Instituicao> getInstituicoes(String nome) throws Exception {
        String sql = "SELECT * FROM tb_instituicao WHERE nm_instituicao LIKE '%" + nome + "%'";
        con = new OracleConnector().getConnection();
        stmt = con.createStatement();
        rs = stmt.executeQuery(sql);
        ArrayList<Instituicao> insts = new ArrayList<Instituicao>();
    ...
    public int setInstituicao(Instituicao inst) throws Exception{
        String sql = "INSERT INTO tb_instituicao VALUES ("+inst.getCd_instituicao()+",'"+inst.getNm_instituicao()+"')";
        con = new OracleConnector().getConnection();
        stmt = con.createStatement();
